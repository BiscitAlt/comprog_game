...
void Map::GenerateNewRoom() {
    ...
    // สุ่มตำแหน่งประตู แก้ทีหลัง
    //switch (GetRandomValue(0, 3)) { 
    //    case 0: data[0][cols/2] = 28; data[0][(cols/2)+1] = 20; data[0][(cols/2)-1] = 21; break;          // ประตูบน
    //    case 1: data[rows-1][cols/2] = 28; data[rows-1][(cols/2)+1] = 14; data[rows-1][(cols/2)-1] = 15; break;     // ประตูล่าง
    //    case 2: data[rows/2][0] = 28; data[(rows/2)+1][0] = 15; data[(rows/2)-1][0] = 21; break;          // ประตูซ้าย
    //    case 3: data[rows/2][cols-1] = 28; data[(rows/2)+1][cols-1] = 14; data[(rows/2)-1][cols-1] = 20; break;     // ประตูขวา
    //}
    switch (GetRandomValue(0, 3)) { 
        case 0: data[0][cols/2] = 28; data[0][(cols/2)+1] = 28; data[0][(cols/2)-1] = 28; break;          // ประตูบน
        case 1: data[rows-1][cols/2] = 28; data[rows-1][(cols/2)+1] = 28; data[rows-1][(cols/2)-1] = 28; break;     // ประตูล่าง
        case 2: data[rows/2][0] = 28; data[(rows/2)+1][0] = 28; data[(rows/2)-1][0] = 28; break;          // ประตูซ้าย
        case 3: data[rows/2][cols-1] = 28; data[(rows/2)+1][cols-1] = 28; data[(rows/2)-1][cols-1] = 28; break;     // ประตูขวา
    }
}
void Map::Draw() {
    for (int y = 0; y < rows; y++) {
...
bool Map::IsDoor(float x, float y) {
    ...
    return data[row][col] == 28; // เช็คว่าเหยียบ ID 28 ไหม
}

void Map::ChangeRoom(int dx, int dy) {
    // 1. เซฟห้องปัจจุบันก่อนย้าย
    RoomData current;
    for(int y=0; y<rows; y++) {
        for(int x=0; x<cols; x++) current.data[y][x] = data[y][x];
    }
    visitedRooms[{currentRoomX, currentRoomY}] = current;

    // 2. ย้ายตำแหน่งพิกัดโลก
    currentRoomX += dx;
    currentRoomY += dy;

    // 3. เช็คว่าเคยมาห้องพิกัด {X, Y} นี้หรือยัง?
    if (visitedRooms.count({currentRoomX, currentRoomY})) {
        // โหลดห้องเดิมกลับมา
        RoomData loaded = visitedRooms[{currentRoomX, currentRoomY}];
        for(int y=0; y<rows; y++) {
            for(int x=0; x<cols; x++) data[y][x] = loaded.data[y][x];
        }
    } else {
        // เป็นห้องใหม่ -> สุ่มสร้างเลย
        GenerateNewRoom();
    }
}